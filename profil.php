...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_FILES["photo"])) {
    ...
    if (!Csrf::verifier($_POST["csrf_token"] ?? null)) {
        ...
        $erreur_photo = "Votre session a expiré, veuillez réessayer.";
 
    } else {
 
        $resultat_photo = $controller->televerserPhoto($_FILES["photo"], $_SESSION["utilisateur_id"]);
 
        if ($resultat_photo["succes"]) {
            $succes_photo = $resultat_photo["message"];
            $utilisateur = $controller->obtenirProfil($_SESSION["utilisateur_id"]); // recharger avec la nouvelle photo
        } else {
            $erreur_photo = $resultat_photo["message"];
        }
    }
}
    ...
    <div class="compte-carte">
      ...
      <div class="compte-entete">
        ...
        <div>
          ...
          <form method="POST" enctype="multipart/form-data" class="formulaire-photo">
            <?= Csrf::champCache() ?>
            <label for="photo" class="lien-photo">Changer de photo</label>
            <input type="file" id="photo" name="photo" accept="image/jpeg,image/png,image/webp" onchange="this.form.submit()" hidden>
          </form>
      ...
      <?php if (!empty($erreur_photo)): ?>
        <div class="message-erreur"><?= htmlspecialchars($erreur_photo) ?></div>
      <?php endif; ?>
 
      <?php if (!empty($succes_photo)): ?>
        <div class="message-succes"><?= htmlspecialchars($succes_photo) ?></div>
      <?php endif; ?>
